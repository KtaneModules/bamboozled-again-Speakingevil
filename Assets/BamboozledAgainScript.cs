...
public class BamboozledAgainScript : MonoBehaviour {
    ...
    void ButtonSet(int b)
    {
        ...
        if (textField[q].Contains(ech[p]))
        {
            buttonVals[b] += 70;
        }
        else if ((p == 0 && q == 64) || (p != 0 && p < 7 && textField[q].Contains(ech[p + 7])) || (p != 14 && p > 7 && textField[q].Contains(ech[p - 7])))
        {
            buttonVals[b] += 35;
        }
        else if(q > 67 || q == 64 || q == 63 || q == 45 || q == 43 || q == 42 || q == 41 || q == 40 || q == 39 || q == 3 || q == 2)
        {
            buttonVals[b] += 5;
        }
        ...
        {
            case 0:
                finalVals[b] = 3 * buttonVals[b] + 2 * (2 * screenVals[0] + screenVals[5]);
                break;
            case 1:
                finalVals[b] = 3 * buttonVals[b] + 2 * (screenVals[0] + screenVals[2] + screenVals[6]);
                break;
            case 2:
                finalVals[b] = 3 * buttonVals[b] + 2 * (2 * screenVals[2] + screenVals[7]);
                break;
            case 3:
                finalVals[b] = 3 * buttonVals[b] + 2 * (screenVals[2] + screenVals[4] + screenVals[5]);
                break;
            case 4:
                finalVals[b] = 3 * buttonVals[b] + 2 * (2 * screenVals[4] + screenVals[6]);
                break;
            case 5:
                finalVals[b] = 3 * buttonVals[b] + 2 * (screenVals[0] + screenVals[4] + screenVals[7]);
                break;
        }
    }
